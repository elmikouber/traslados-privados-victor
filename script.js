...
function construirMensajeReserva(datos, promedio, minimo, maximo, confirmado, reserva, refSolicitud = null) {
    ...
    if (refSolicitud) {
        lineasReserva.push(`🔖 REF. SOLICITUD: ${refSolicitud}`, "");
    }
...
async function enviarReservaWhatsApp(event) {
    ...
    const continuarBtn = document.getElementById("reservaContinuar");
    if (continuarBtn) {
        continuarBtn.disabled = true;
        continuarBtn.textContent = "Guardando solicitud…";
    }

    let refSolicitud = null;

    if (typeof window.guardarSolicitudFirebase === "function") {
        try {
            refSolicitud = await window.guardarSolicitudFirebase({
                datos,
                promedio,
                minimo,
                maximo,
    ...
    if (continuarBtn) {
        ...
        continuarBtn.innerHTML = `
            ...
            Continuar con la reserva por WhatsApp
        `;
    }

    const mensaje = construirMensajeReserva(datos, promedio, minimo, maximo, confirmado, reserva, refSolicitud);
